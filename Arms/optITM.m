...
T_1064 = 0.014;         % desired ITM transmission @ 1064
R_1064 = 1 - T_1064;

T_532 = 0.01;           % desired ITM transmission @ 532
R_532 = 1 - T_532;


y = 10 * (abs(R1(2)) - R_1064)^2 +...
    1 * (abs(R1(1)) - R_532)^2;
...
legend('R','T')
grid
axis([min(lambda_real) max(lambda_real) 1e-3 1])
title('Calculated ITM HR Reflectivity')
...
text(1300,0.1*1.3^3,['R @ 1064 = ' num2str(R1(2))])
text(1300,0.1*1.3^2,['T @ 1064 = ' num2str(T1(2))])
text(1300,0.1*1.3^-1,['R @  532 = ' num2str(R1(1))])
text(1300,0.1*1.3^-2,['T @  532 = ' num2str(T1(1))])
