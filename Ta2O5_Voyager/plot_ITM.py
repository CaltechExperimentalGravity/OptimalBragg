...
Example usage:
    plotLayers.py aLIGO_ITM_20layers.mat
will take the coating design in aLIGO_ITM_20layers.mat and make a plot of the
...
if len(sys.argv) == 1:
    fname = max(glob.iglob('Data/ITM/*Layers*.hdf5'), key=os.path.getctime)
    # fname = fname[5:] # rm 'data' from the name
else:
    # For example fname = 'ITM_Layers_190519_1459.hdf5'
...
def plot_layers(save=False):
    ...
    ax2[1].bar(layers[:-1:2], 1e9*L[::2], width=1e6*L[::2],
            ...
            align='edge', color='xkcd:bright teal',
                  alpha=0.4, label=R'$\mathrm{SiO}_2$')
...
if __name__ == '__main__':    
    if __debug__:
        print('Make plots...')
    main(layers=False, trans=False, noise=False)
