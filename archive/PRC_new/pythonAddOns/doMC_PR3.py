...
#cov[0,0] = 0.05 #Larger perturbation for angle of incidence
cov = np.dot(cov,cov)
...
for jj in range(nSamples):
    ...
    [Gamma5p, t1] = multidiel1(n_IRs, Ls*n_IRs[1:-1], 1,theta=aoi,pol='tm')
    Tp_IR[jj]=(1. - np.abs(Gamma5p)**2)
    surfField[jj]=surfaceField(Gamma5p,Ei=1)
    [Gamma5p, t2] = multidiel1(n_greens, Ls*n_greens[1:-1], 0.5,theta=aoi, pol='tm')
    Rp_green[jj] = (np.abs(Gamma5p)**2)
    [Gamma5s, t2s] = multidiel1(n_greens, Ls*n_greens[1:-1], 0.5, theta=aoi)
    Rs_green[jj] = (np.abs(Gamma5s)**2)

samples = np.vstack((1e6*Tp_IR,1e2*Rp_green,1e2*Rs_green,surfField))
