...
[temp, ~] = multidiel1(n_temp, L, lambda, aoi, 'tm');
tempT = 1-abs(temp).^2;
tempR = abs(temp).^2;
err.Tp.n1_plus = ((tempT - Tp)/(Tp))/0.01; 
err.Rp.n1_plus = ((tempR - Rp)/(Rp))/0.01; 
[temp, ~] = multidiel1(n_temp, L, lambda, aoi, 'te');
tempR = abs(temp).^2;
