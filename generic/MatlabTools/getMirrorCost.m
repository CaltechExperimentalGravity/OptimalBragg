...
[Gammas, ~]   = multidiel100(n_IR, L, lambda_IR, aoi, 'te');
Rp_IR = abs(Gammap).^2;
Tp_IR = 1 - Rp_IR;
Rs_IR = abs(Gammas).^2;
Ts_IR = 1 - Rs_IR;
...
[Gammas, ~]   = multidiel100(n_green, L_green, lambda_green, aoi_green, 'te');
...
yy = [yy heaviside(Rs_green - R_2s)*abs((Rs_green - R_2s)/R_2s)^1];  % match the T @ lambda/2, s-pol
...
if yy(1)<2 && yy(2)<2 && yy(3)<2
    y = sum(yy(1:4) .* weights(1:4));   %Terms included by default...
    if glob_param.include_thermal
        ...
        y = y + weights(5) * S;
        yy(5) = weights(5) * S;                           % minimize the Brownian noise
